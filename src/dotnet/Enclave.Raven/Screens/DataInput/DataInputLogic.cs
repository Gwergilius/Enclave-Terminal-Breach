...
/// Pure (no I/O, no rendering) — injected into <see cref="DataInputViewModel"/> and tested independently.
/// </summary>
internal static class DataInputLogic
{
    /// <summary>
    /// Splits <paramref name="line"/> into whitespace-delimited, trimmed, non-empty tokens.
    /// </summary>
    ...
    /// </summary>
    /// <returns>List of error messages (one per failed add/remove); empty when there are no errors.</returns>
    ...
    /// <c>CandidateListText</c> is <see langword="null"/> when there are no candidates or the word length is unknown.
    /// </summary>
